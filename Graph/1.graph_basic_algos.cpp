...
class graph {
public:
	map<T, list<pair<T,int> > > adjlist;// storing way: node:node ,path weight 
	...
	//Implementation of BFS: Single Source Shortest Path(SSSP) of a unweighted graph
	//Distance every other node from source node
	void bfs_sssp(T src)
	{
		map<T, int> distance;
		map<T, T> parent;
